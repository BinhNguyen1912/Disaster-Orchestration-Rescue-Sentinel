...
> **Lần cập nhật gần nhất:** 2026-07-04
> **Người cập nhật:** AI Assistant (cập nhật sau khi tối ưu hóa UX/UI, đồng bộ trạng thái cứu hộ và tính toán khoảng cách thực tế)
> **Trạng thái tổng:** 🟢 **Phase 3.5 — Spatial NNS + TomTom Traffic Integration + WebSockets + UX & State Sync**
...
|------|---------|
| 2026-07-04 | **Vai trò Động, Trực tiếp Sửa, Sắp xếp, Xuất Excel/PDF, Sidebar Co giãn, Khóa Quy trình Vị trí, Đồng bộ Trạng thái & Thao tác hàng loạt, Tính khoảng cách Haversine & Lưu Database**: 1) Phân quyền trực tiếp qua Details Pane dropdown với vai trò động `/roles`. 2) Loại bỏ thanh tabs đếm số lượng stats, căn lề lại bộ lọc. 3) Hỗ trợ menu sắp xếp dropdown theo Họ tên, Ngày tạo, SĐT. 4) Tạo component xuất dữ liệu `<ExportMenu />` (Excel/PDF). 5) Nâng NestJS log level auto-dispatch v6 hiển thị bảng điểm xếp hạng. 6) Thiết kế sidebar cấu hình thu gọn `w-16`/`w-64` kèm micro-animations. 7) Khóa quy trình nhập địa chỉ/ghim bản đồ cho đến khi chọn Phường/Xã để tránh sai lệch dữ liệu, tự động flyTo về trung tâm phường/xã. 8) Khắc phục lệch trạng thái bằng cách tự động cập nhật trạng thái đội sang `BUSY` trên backend khi SOS đạt trạng thái `ON_SITE` (tiếp cận), đồng bộ ánh xạ trạng thái `DISPATCHED` (Đang tiếp cận) và hiệu ứng vòng tròn Leaflet. 9) Bổ sung select dropdown trạng thái tối giản dạng chữ trên table. 10) Thiết lập tính năng Cập nhật trạng thái hàng loạt (Bulk Status Update) qua checkbox table và thanh hổ phách. 11) Tích hợp công thức địa lý Haversine tính toán khoảng cách thực tế giữa SOS và Đội cứu hộ trên Frontend (Detail Modal + Disaster Sidebar) và lưu trữ trực tiếp vào cột cơ sở dữ liệu `distance_km` mới khi điều phối (cả tự động và thủ công). |
